...
RESNET_MODEL_PATH = os.path.join(BASE_DIR, "resnet18_mask_classifier.pth")
...
# ============================================================
# PART 5: Dataset class for PyTorch (Phylicia)
# ============================================================
class MaskFolderDataset(Dataset):
    """
    Simple dataset that reads from:
      - green_mask_dir (Mask, label=1)
      - red_nomask_dir (No Mask, label=0)

    and returns (image_tensor, label).
    """

    def __init__(self, green_mask_dir: str, red_nomask_dir: str, transform=None):
        self.samples = []
        self.transform = transform

        # label 1 = Mask
        for fname in os.listdir(green_mask_dir):
            if fname.lower().endswith((".png", ".jpg", ".jpeg")):
                self.samples.append((os.path.join(green_mask_dir, fname), 1))

        # label 0 = No Mask
        for fname in os.listdir(red_nomask_dir):
            if fname.lower().endswith((".png", ".jpg", ".jpeg")):
                self.samples.append((os.path.join(red_nomask_dir, fname), 0))

        if not self.samples:
            raise RuntimeError("MaskFolderDataset found no images.")
    ...
    def __getitem__(self, idx):
        ...
        img_bgr = cv2.imread(path)
        if img_bgr is None:
            # fall back: return a black image if something goes wrong
...
# ============================================================
# PART 6: ResNet18 model with custom 
# ============================================================
class MaskResNet18(nn.Module):
    """
    Option B:
      ResNet18 backbone (pretrained on ImageNet)
      + custom classification head for 2 classes: [NoMask, Mask]

    This is your "custom CNN" for the project.
    """
...
# ============================================================
# PART 6: TRAINING LOOP For ResNet Model : Phylicia
# ============================================================
def train_resnet_model(epochs: int = 10, batch_size: int = 32, lr: float = 1e-3) -> MaskResNet18:
    """
      1) Download + copy Kaggle dataset.
      2) Build FaceMaskDataset (images + XML).
      3) Create category + annotated folders (green with-mask / red without mask).
      4) Build MaskFolderDataset (PyTorch) from those folders.
      5) Split into train/val.
      6) Train ResNet18 + custom head on GPU/CPU.
      7) Save best model to RESNET_MODEL_PATH.
    """
    ...
    full_mask_ds = MaskFolderDataset(green_mask_dir, red_nomask_dir, transform=train_transform)
    n_total = len(full_mask_ds)
    n_val = max(1, int(0.2 * n_total))
    n_train = n_total - n_val

    train_ds, val_ds = random_split(full_mask_ds, [n_train, n_val])
    ...
    model = MaskResNet18(num_classes=2, pretrained=True, freeze_backbone=True).to(device)
    ...
    for epoch in range(epochs):
        ...
        batch_bar = tqdm(train_loader, desc=f"[ResNet] Epoch {epoch+1}/{epochs}", unit="batch")
        ...
        if epoch_val_acc > best_val_acc:
            ...
            print(f"  Best ResNet Model '{RESNET_MODEL_PATH}' (Val Acc={best_val_acc:.4f})")

    print(f"\n=== BEST RESNET VAL ACCURACY: {best_val_acc:.4f} ===\n")
    return model
...
# ============================================================
# PART 9: LIVE WEBCAM DETECTION using ResNet18
# ============================================================
def run_webcam_detection_resnet(model: MaskResNet18):
    """
    Live webcam detection using:
      - YuNet or Haar for face detection (same as Roy)
      - ResNet18 + custom head for Mask / No Mask classification
    """
    model.to(device)
    model.eval()
    ...
    print("\n[Webcam-ResNet] Starting live mask detection. Press 'q' to quit.\n")
    ...
    while True:
        ...
        for (x, y, w_box, h_box) in faces:
            ...
            p_no = float(probs_t[0, 0].item())
            p_mask = float(probs_t[0, 1].item())

            # Reuse Roy's conservative decision rule
            probs_np = np.array([[p_no, p_mask]], dtype=np.float32)
            pred = decide_label_from_probs(probs_np)
            ...
            print(
                f"[Webcam-ResNet] {label} | p_no={p_no:.2f} | p_mask={p_mask:.2f} | "
                f"inference={infer_time_ms:.2f} ms"
            )
        ...
        cv2.imshow("Face Mask Detection (ResNet) - Press 'q' to quit", frame)
        if cv2.waitKey(1) & 0xFF == ord("q"):
            break

    print(f"\n[Webcam-ResNet] Average FPS: {fps:.2f}")
    cap.release()
    cv2.destroyAllWindows()
...
def main():
    ...
    parser.add_argument(
        ...
        default="baseline",
        help="Which model to use: 'baseline' or 'resnet'.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    if args.mode == "train":
        if args.model == "baseline":
            ...
            print("\nTraining complete. Launching webcam(baseline)...")
            run_webcam_detection(model)
        else:
            print("\n=== MODE: TRAIN RESNET ===")
            model = train_resnet_model(epochs=args.epochs)
            print("\nResNet training complete. Launching webcam (ResNet)...")
            run_webcam_detection_resnet(model)   

    elif args.mode == "detect":
        if args.model == "baseline":
            print("\n=== MODE: DETECT baseline (Webcam only, no training) ===")
            ...
            print("\n=== MODE: DETECT RESNET ===")
            model = MaskResNet18(num_classes=2, pretrained=False, freeze_backbone=False).to(device)
            if not os.path.exists(RESNET_MODEL_PATH):
                raise FileNotFoundError(
                    f"ResNet model file '{RESNET_MODEL_PATH}' not found.\n"
                    "Run once with '--mode train --model resnet' to create and save the model."
                )
            state_dict = torch.load(RESNET_MODEL_PATH, map_location=device)
            model.load_state_dict(state_dict)
            print(f"Loaded ResNet model from '{RESNET_MODEL_PATH}'. Launching webcam...")
            run_webcam_detection_resnet(model)
